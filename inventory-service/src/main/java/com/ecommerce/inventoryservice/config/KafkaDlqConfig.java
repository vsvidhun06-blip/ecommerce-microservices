...
/**
 * Dead-letter routing for the inventory service's Saga consumers.
 *
...
public class KafkaDlqConfig {
     ...
     * (InventorySagaHandler, the recoverer below) unsatisfied. Declare it explicitly
     * over the still-auto-configured producer factory to restore it.
     */
    @Bean
    ...
    public NewTopic orderCreatedDlt() {
        return TopicBuilder.name(KafkaTopics.ORDER_CREATED + DLT_SUFFIX).partitions(1).replicas(1).build();
    }

    @Bean
    public NewTopic orderCancelledDlt() {
        return TopicBuilder.name(KafkaTopics.ORDER_CANCELLED + DLT_SUFFIX).partitions(1).replicas(1).build();
    }
}
